...
export default function QuizResult() {
    ...
    const deltaPoints   = attempt.rank_points_awarded;
    const totalFromQuiz = summary?.attempts
        .filter(a => a.status === 'completed')
        .reduce((sum, a) => sum + (a.rank_points_awarded || 0), 0) ?? deltaPoints;
                ...
                <div className="quiz-result-hero" style={{ borderColor: passed ? 'color-mix(in srgb, var(--emerald) 40%, transparent)' : 'color-mix(in srgb, var(--coral) 40%, transparent)' }}>
                    ...
                    {isRetake && (
                        <div style={{ fontSize: 12, color: 'var(--text3)', marginBottom: 6 }}>
                            Attempt {attempt.attempt_number} of {quiz.max_attempts === 0 ? '∞' : quiz.max_attempts}
                        </div>
                    )}
                    ...
                    <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4, 1fr)', gap: 2, background: 'var(--border)', borderRadius: 'var(--r)', overflow: 'hidden', marginBottom: 20 }}>
                        {[
                            ...
                            { label: 'Time',     value: timeTaken,                              color: 'var(--text)' },
                            {
                                ...
                                value: deltaPoints > 0 ? `+${deltaPoints}` : (isRetake ? '—' : `+${deltaPoints}`),
                                color: deltaPoints > 0 ? 'var(--champagne)' : 'var(--text3)',
                            },
                        ].map(s => (
                    ...
                    {/* Retake ranking explanation */}
                    {isRetake && (
                        <div style={{ fontSize: 13, color: 'var(--text3)', marginBottom: 16, padding: '10px 14px', background: 'var(--bg)', borderRadius: 'var(--r)', textAlign: 'left' }}>
                            {deltaPoints > 0
                                ? `✅ Your score improved! +${deltaPoints} rank points awarded (improvement over your previous best).`
                                : `ℹ Your score didn't improve over your previous best (${Math.round(summary?.best_percentage ?? 0)}%). No additional rank points awarded this attempt.`
                            }
                            {' '}Total rank points earned from this quiz: <strong style={{ color: 'var(--champagne)' }}>+{totalFromQuiz}</strong>
                        </div>
                    )}
                    ...
                    {attempt.ai_flagged && (
                        <div style={{ background: 'color-mix(in srgb, var(--coral) 10%, transparent)', border: '1px solid color-mix(in srgb, var(--coral) 30%, transparent)', borderRadius: 'var(--r)', padding: '12px 16px', fontSize: 13, color: 'var(--coral)', textAlign: 'left', marginBottom: 16 }}>
                            ⚠️ <strong>{flaggedCount} answer{flaggedCount > 1 ? 's' : ''}</strong> flagged for possible AI assistance. Points for those answers are withheld pending admin review.
                        </div>
                    )}

                    <div style={{ fontSize: 13, color: 'var(--text3)' }}>
                        {passed
                            ? `You passed with ${Math.round(pct)}%. Passing score was ${quiz.passing_score}%.`
                            : `Passing score is ${quiz.passing_score}%. You scored ${Math.round(pct)}%. Review below and try related forum topics.`
                        }
                    </div>
                </div>
                ...
                <div style={{ display: 'flex', gap: 12, marginTop: 32, flexWrap: 'wrap' }}>
                    {summary?.can_attempt && (
                        <Link href={`/quiz/${quiz.slug}`} className="btn btn-primary">
                            {summary.completed_count > 0 ? 'Retake Quiz' : 'Try Again'}
                        </Link>
                    )}
                    <Link href={`/quiz/${quiz.slug}`} className="btn btn-ghost">Quiz Details</Link>
                    <Link href="/quiz" className="btn btn-ghost">Browse Quizzes</Link>
                    {quiz.tag && (
        ...
        <div style={{ background: 'var(--surface)', border: `1px solid ${borderColor}`, borderLeft: `3px solid ${borderColor}`, borderRadius: 'var(--r-lg)', padding: 20, marginBottom: 12 }}>
            <div style={{ display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between', gap: 12, marginBottom: 10 }}>
                ...
                <div style={{ display: 'flex', gap: 6, flexShrink: 0 }}>
                    {isFlagged && (
                        <span style={{ fontSize: 11, padding: '2px 8px', borderRadius: 999, background: 'color-mix(in srgb, var(--champagne) 15%, transparent)', color: 'var(--champagne)', border: '1px solid color-mix(in srgb, var(--champagne) 30%, transparent)', fontWeight: 700 }}>
                            AI Flagged
                        </span>
                    )}
            ...
            {q?.type === 'mcq' && answer.selected_option && (
            ...
            {q?.type === 'coding' && answer.answer_text && (
            ...
            {isFlagged && (
                <div style={{ fontSize: 12, color: 'var(--champagne)', marginTop: 8 }}>
                    ⚠ Points withheld — pending admin review.
                </div>
            )}
        </div>
    );
