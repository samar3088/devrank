...
- **DB-backed** sessions + queue (`SESSION_DRIVER`/`QUEUE_CONNECTION=database`); shared-host cron runs `schedule:run` + `queue:work --stop-when-empty` each minute (no persistent worker). Mail = Mailtrap sandbox for now. Judge0/AI off (MCQ-only); GitHub OAuth deferred to a post-deploy OAuth app on the www URL.
