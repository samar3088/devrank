...
## AI mock interview (#5) — BUILT (data moat)
- **Practise the real rounds companies run**, built from crowd-sourced interview-board reports. `MockInterviewService`: `prepData(company, role)` aggregates real `rounds_detail` (types/difficulty/descriptions) + `tips`; `start()` creates a `mock_interviews` session (JSON `transcript`); `complete()` grades it. Candidate routes `/mock-interview{,/{mock},/{mock}/submit}` (candidate-only, `throttle:10,1`); pages `MockInterview/Index|Show`; entry via nav "🎤 Mock Interview" + interview-board "Practice with AI".
- **Two modes (gated on `config('devrank.ai.enabled')`):** AI **on** → questions AI-generated grounded in the real data + each answer scored 0–10 with feedback + an overall readiness score/summary (reuses the Anthropic call pattern from `AiScoringService`). AI **off** → "prep mode": questions derived directly from the real rounds, real tips shown, **no API call/cost** (`ai_graded=false`, no score).
...
## Verified hire outcomes + salary transparency (#11) — BUILT
- **Two-sided hire confirmation + aggregate salary transparency.** `HireService` + `hire_outcomes` table (one row per `job_application`, unique). Flow: a company **records a hire** from the Applicants view (`POST /company/applications/{application}/hire` → `Company\JobController@hire` → `recordHire`) capturing the real offer (`offered_salary`/`salary_currency`/`salary_period`/`starts_on`); the application moves to the **new terminal `hired`** pipeline stage (enum extended in the migration), the SLA clock is stamped, and the candidate is notified. The candidate then **confirms or declines** on `/hires` (`Hires/Index.jsx`; `POST /hires/{hireOutcome}/confirm|decline`, candidate-only) — confirm → `status=verified`; decline → `status=declined` **and the application rolls back to `offered`** (pipeline stays honest). Statuses: `pending → verified | declined`.
- **Salary is candidate-consented + k-anonymised.** At confirm time the candidate opts into sharing (`salary_shared`, default false). Public **`GET /salaries`** (`Salaries/Index.jsx`, `HireController@salaries`) shows **aggregate-only** medians/p25–p75/ranges from `verified` + `salary_shared` outcomes — never an individual figure. Every bucket (overall / by experience level / by role) is **suppressed until it has `config('devrank.hires.min_sample')` (default 3) shared offers**. Salaries are **annualised** (monthly × 12) and **grouped per currency** (never blended); the largest-sample currency is reported.
- **Trust signal, not score mutation:** verified hires surface as a **"Verified Hires" count** on the public company profile (`PublicProfileService::getCompanyProfile` → replaced the old hardcoded "42 Platform Hires" mock) and company dashboard stats. The penalty-based `trust_score` formula (ghosting + SLA) is intentionally **left untouched** to preserve its tested invariants.
- **DPDP:** the offer figure is personal data — added to `AccountService` **export** (`hire_outcomes`) and **erasure** (nulls `offered_salary` + sets `salary_shared=false` so it drops out of transparency aggregates, keeping only the anonymised hire fact). Salary sharing is explicit opt-in (consent).
- **Notifications:** `hire_recorded` (→ candidate), `hire_verified` / `hire_declined` (→ company) via `NotificationService::notify`.
- Nav: **"Salaries"** public link (all roles) + candidate **"🎉 My Hires"** dropdown item; candidate dashboard shows a **pending-hire confirmation card** (`stats.pending_hires`). Config: `config('devrank.hires.min_sample')` (env `DEVRANK_SALARY_MIN_SAMPLE`).

## Deployment (live on shared hosting — samatasrivastva.com)
- **Live site:** `https://www.samatasrivastva.com` (cPanel shared host, `/home3/samarev5/samatasrivastva.com`). Forced HTTPS+www via a root `.htaccess` that routes to `public/` (app kept whole in the domain folder — host locks the docroot). PHP 8.2, Composer 2.6, no Node on server (assets prebuilt + shipped).
- **Not indexed:** `robots.txt` Disallow all + `<meta robots noindex>` + `X-Robots-Tag` header, all gated on **`config('app.indexable')`** (env `APP_INDEXABLE`, default false). Flip to true + edit robots.txt to go public.
- **DB-backed** sessions + queue (`SESSION_DRIVER`/`QUEUE_CONNECTION=database`); shared-host cron runs `schedule:run` + `queue:work --stop-when-empty` each minute (no persistent worker). Mail = Mailtrap sandbox for now. Judge0/AI off (MCQ-only). **GitHub OAuth configured on production** — OAuth app registered, `GITHUB_CLIENT_ID/SECRET` set in the server `.env`, callback `https://www.samatasrivastva.com/auth/github/callback`; the Connect-GitHub card is live on the candidate dashboard.
- **Deploy workflow (delta):** edit + commit in the working folder → `npm run build` if JS/CSS changed → run `C:\Users\samsr\devrank-deploy\sync.ps1` → it writes ONLY the changed files (+ rebuilt `public/build`, minus dev/secret/compiled-source/.md) into `devrank-deploy\changes\` → upload that folder's contents (overlay) → on server `php artisan config:clear && config:cache && route:cache && view:cache` → `git rev-parse HEAD > devrank-deploy\.last-deploy` to advance the baseline. Local-only `devrank-deploy\docs\` holds the credentials + features Word docs (never uploaded). Full runbook: [`docs/DEPLOYMENT.md`](docs/DEPLOYMENT.md).

## UI conventions (nav / footer / legal — added this pass)
- **Public legal/info pages:** `/privacy`, `/terms`, `/cookies`, `/about` (all `LegalController`, pages in `resources/js/Pages/Legal/*`, wrapped in `MainLayout` + `FullFooter`). Footer legal links point to these; Company column is **About Us only** (Blog/Careers/Press removed).
- **Footer** (`Components/Footer.jsx`): `FullFooter` (marketing, dynamic year, `SocialLinks` — placeholder `#` hrefs until real URLs given) is placed **outside** page content containers (putting it inside a padded `.container`/`.profile-container` leaves a blank bar below it). Signed-in shells that lack a marketing footer (`AdminLayout`, `CompanyLayout`) render a slim `.app-footer` copyright bar instead.
- **Top nav** (`MainLayout`): logo → `/dashboard` when signed in (else `/`); admin pages get a full-width bar (`navbar--admin`) with role nav links (admin is no longer blank); the user pill shows a `▾` caret; the dropdown has no separate "Admin Panel" item (Dashboard already redirects admins there). `NotificationBell` always shows a count badge (muted `.is-zero` at 0).
- **Job board filters are multi-select** (checkboxes, not radios): `job_type`/`work_mode`/`experience` are arrays → `whereIn` in `JobService::getPublicJobs`; `JobBoardController` normalises each to an array; `SavedSearch::matchesJob` accepts array-or-scalar filters.

## Roadmap status (11-item plan; see FEATURE-STATUS.md for detail)
**Built ✅ (11/11 — COMPLETE):** #1 Judge0 code execution · #2 verifiable rank credentials · #3 bias-reduced hiring · #4 smart matching · #5 AI mock-interview · #6 GitHub import · #7 in-app notifications · #8 seasons/leagues/weekly challenges · #9 skill paths · #10 response SLAs · #11 verified hire outcomes + salary transparency.
...
**Resolved:** the `monaco-editor` transitive **dompurify** advisory is fixed via an npm **`overrides`** block forcing `dompurify@^3.4.15` (patched) → `npm audit` = **0 vulnerabilities**. (Monaco is CDN-loaded at runtime, so this was dev-dependency hygiene, not the forum XSS boundary — that's server-side HTMLPurifier.)
